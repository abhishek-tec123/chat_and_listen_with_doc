...
MODEL_ID = os.environ['MODEL_ID']
PDF_DIRECTORY = "/Users/macbook/Desktop/texttoaudio/data2"  # Directory with multiple PDFs
...
if prompt_input:
    ...
    with st.expander("Document Similarity Search"):
        ...
        for i, doc in enumerate(response["context"]):
            ...
            # Display the document index, filename, and page number
            st.write(f"Reference Document Index: {document_index+1} | Source Document: {filename} | Page Number: {page_number}")
            st.write(doc.page_content)
            st.write("--------------------------------")
